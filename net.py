...
class Attention_3DGS(nn.Module):
    def __init__(self,
        ...
        self.gen_gaussian_ellipsoids = Ellipsoids(num_points, matrix_dim, min_val, max_val, device=device)
        self.paper_self_attention = Self_Attention_Block(seq_len, input_dim, embed_dim)
        self.brush_self_attention = Self_Attention_Block(num_points, self.gen_gaussian_ellipsoids.len, embed_dim)
        self.draw_cross_attention = Cross_Attention_Block(seq_len, num_points, embed_dim, num_layers, num_heads,
                                                          dropout)

    def forward(self, x):
        x = self.fc_seq(x)  # (batch, 6) -> (batch, seq_len*6)
        q = x.view(-1, self.seq_len, self.input_dim)  # (batch, seq_len*6) -> (batch, seq_len, 6)
        kv = self.gen_gaussian_ellipsoids().unsqueeze(0).expand(x.size(0), -1, -1)  # (num_points, 9)->(batch,
        # num_points, 9)
